...
{
    ui->setupUi(this);
    this->setWindowTitle("COMPROMISSOS");
...
void CompromissosWindow::carregarDaPosicao(long pos)
{
    ...
    if (valid)
    {
        ...
        ui->lineEditMes->setText(QString::fromStdString(temp.data.getMesString()));
...
void CompromissosWindow::on_pushButtonRemover_clicked()
{
    ...
    if (ptr_usuarios->contentAt(Sessao::getPosicao()).compromissos.iterate(temp, this->posicao))
...
void CompromissosWindow::resetLines()
{
    ui->lineEditAno->setText("");
    ui->lineEditMes->setText("");
    ui->lineEditDia->setText("");
    ...
    ui->plainTextEditDescricao->document()->setPlainText("");
}
